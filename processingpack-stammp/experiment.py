...
class Experiment:
    def __init__(self, description, root, operator, repoName = 'Repo'):
        ...
        Arguments:
            (str) description: User-defined experimental description
    ...
    def _initializeLogger(self):
        ...
        formatter = logging.Formatter('%(levelname)-8s %(message)s', datefmt='%Y/%m/%d %H:%M:%S')
    ...
    def addDevices(self, devices):
        ...
        def add(d):
            if isinstance(d, Device):
                ...
                raise ValueError(u'Must add an experimental Device object')
        if isinstance(devices, tuple) or isinstance(devices, list):
            for d in devices:
                if any([d == device for device in self.devices]):
                    logging.warn('Device already added |  Device: {}'.format(d.__str__()))
                else:
                    add(d)
...
class Device:
    def __init__(self, setup, dname, dims, pinlist, corners, operators = 'FordyceLab', attrs = None):
        """
    ...
    def __eq__(self, other):
        if isinstance(other, Device):
            return (self.setup == other.setup) and (self.dname == other.dname) and (self._corners == other._corners)
        else:
